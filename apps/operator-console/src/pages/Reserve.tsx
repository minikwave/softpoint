  ...
  return (
    <>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
        사용자에게 보이는 SoftPoint(SP) 합계(미사용 부채)입니다. 은행·온체인 대사는 별도 워커가 필요합니다.
      </p>
      ...
      {data && (
        <div className="card">
          <dl className="detail-dl" style={{ display: 'grid', gridTemplateColumns: '10rem 1fr', gap: '0.5rem' }}>
            <dt>총 잔액</dt>
            <dd>{fmt(data.total_balance)} SP</dd>
            <dt>예약(전환 등)</dt>
            <dd>{fmt(data.total_reserved)} SP</dd>
            <dt>계정 수</dt>
            <dd>{data.account_count}</dd>
            <dt>누적 ISSUE 합</dt>
            <dd>{fmt(data.total_issued)} SP</dd>
            <dt>누적 SPEND 합</dt>
            <dd>{fmt(data.total_spent)} SP</dd>
            <dt>교환 완료 건수</dt>
