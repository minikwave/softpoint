...
| **Phase 0+ (마감·하드닝)** | 사용자 API 인증(JWT/세션), conversion/request 멱등, Spend/Issue 멱등 정책 문서화, DB 격리 수준 운영 가이드, paypoint-api에 남은 write 감사 누락 점검 | **남음** |
...
| P0-2 | `POST /v1/paypoint/conversion/request` 멱등 키 | 주문/요청 ID 기반 |
