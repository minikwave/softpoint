...
> **구현 진행도**: Phase 0 Step 체크리스트는 **실제 코드 기준으로 갱신**되어 있습니다. 남은 작업·머지/모음/마켓 확장은 **§10 남은 투두 및 확장 로드맵**을 참고하세요. 한 줄 요약은 [README.md](./README.md)의 「문서 대비 구현 현황」「제품 관점: 머지(통합)와 모음(쌓음)」을 참고하세요. **브랜드 가칭(쌓음·saum·모음)** 은 [docs/PRODUCT_NAMING.md](./docs/PRODUCT_NAMING.md).
...
| **Phase 1a — 머지·모음 UX** | 멀티 소스 `issue` 연동(어댑터는 별도 서비스), 트랜잭션 `source`/메타 표준, 디앱 적립·내역 출처 표시·필터, EarnMap/가맹 API 연동 | **대부분 완료** (§10.2·§7) |
...
| **Phase 1d — 포인트앱형 허브 UX** | 한 화면에서 잔액·사용처·사용/적립 내역·추가 적립 경로(벤치마크: 주요 포인트 앱) | **계획** (§10.8 `UX-*`) |
...
아래는 README 「제품 관점: 머지(통합)와 모음(쌓음)」에서 정리한 갭과, Phase 0+ 이후 작업을 **실행 가능한 투두**로 묶은 것이다. **포인트 상품권(선불 할인)**, **PCI 충전→수집품 Spend**, **수요자 리워드**, **포인트앱형 정보 허브**, **메신저(텔레그램 우선)** 는 §10.5–10.9에 반영했다. 우선순위는 팀에서 조정한다.
...
이 문서는 **Phase 0는 대부분 완료**된 상태를 전제로 하며, **§10**부터 순서대로 백로그를 잡으면 머지·모음·마켓·선불 상품권·수집품·포인트앱형 UX·메신저까지 제품 요구를 체계적으로 반영할 수 있습니다.
