...
export interface AdminIssueRes {
  accountId: string;
  userId: string;
  amount: string;
  txId: string;
...
export type SoftPointAdminClientOptions = HttpClientOptions & { adminApiKey: string };

export function createSoftPointAdminClient(options: SoftPointAdminClientOptions) {
  const { request } = createHttpClient({ ...options, adminApiKey: options.adminApiKey });
  ...
  return {
    getUsers(params?: { user_id?: string; status?: string; limit?: string }) {
      ...
      return request<{ items: AdminUserItem[]; count: number }>(
    ...
    issueCredit(body: {
      ...
      return request<AdminIssueRes>('/v1/admin/credits/issue', {
        method: 'POST',
    ...
    getConversions(params?: { status?: string; user_id?: string; limit?: string }) {
      ...
      if (params?.limit) q.set('limit', params.limit);
      const query = q.toString();
      return request<{ items: ConversionItem[]; count: number }>(
        `/v1/admin/conversions${query ? `?${query}` : ''}`
      );
    },
    ...
    getPolicies(params?: { policy_id?: string; status?: string; limit?: string }) {
      ...
      if (params?.limit) q.set('limit', params.limit);
      const query = q.toString();
      return request<{ items: PolicyItem[]; count: number }>(
        `/v1/admin/policies${query ? `?${query}` : ''}`
      );
    },
    ...
    getExceptions(params?: { status?: string; user_id?: string; limit?: string }) {
      ...
      if (params?.limit) q.set('limit', params.limit);
      const query = q.toString();
      return request<{ items: ExceptionItem[]; count: number }>(
        `/v1/admin/exceptions${query ? `?${query}` : ''}`
      );
    },
    ...
    getAuditLogs(params?: {
      ...
      const query = q.toString();
      return request<{ items: AuditLogItem[]; next_cursor: string | null }>(
...
export type SoftPointAdminClient = ReturnType<typeof createSoftPointAdminClient>;
