...
PayPoint(saum) 레포 기준 **2026년 4월** 시점 스냅샷. **브랜드 전제: 쌓음(saum)** — [PRODUCT_NAMING.md](./PRODUCT_NAMING.md). 로드맵 ID는 [PROJECT_ANALYSIS_AND_PLAN.md §10](../PROJECT_ANALYSIS_AND_PLAN.md)(`PG-` `DC-` `UX-` `MSG-` 포함).
...
| 경로 | 내용 |
|------|------|
| `/` | 랜딩: 공통 헤더(제품·개발자·디앱), MVP 현황 pill, 디앱/가게 진입 |
| `/product` | 제품: 쌓음/PayPoint, MVP·한계·로드맵(레포 링크, `VITE_REPO_URL`로 원격 지정 가능) |
| `/developers` | 개발자: 빠른 시작, OpenAPI·문서 목록 링크 |
...
### 6.4 머지(통합 포인트)·쌓음(saum) 앱 스토리

- **머지**: 제휴·체인·캠페인이 각각 `issue`(또는 정책)로 유입 → **하나의 잔액**으로 표시. 어댑터는 **별도 서비스**로 두는 전제.
- **쌓음(saum) 디앱**: 한 앱 안에서 적립·내역·지도·결제·정산·가게를 묶어 **“모아 쓰고 정산”** 데모. **소스별 탭**(월렛A·제휴B)은 없음 → 향후 제품화 시 선택.
...
- [openapi.yaml](./openapi.yaml) — REST API OpenAPI 3 스켈레톤 (엔드포인트·인증·주요 스키마)  
- [MVP_REMAINING_AND_RECONCILIATION.md](./MVP_REMAINING_AND_RECONCILIATION.md) — **MVP 잔여 투두**, 은행·온체인·PCI 대사/모니터링 **레포 내 부재 확인**  
