...
- **정책 워크플로 (Admin)**: `GET /v1/admin/policies`, `POST .../policies/draft`, `.../submit`, `.../approve`, `.../activate` — 상태 `DRAFT`→`SUBMITTED`→`APPROVED`→`ACTIVE`; 동일 `policy_id`의 기존 `ACTIVE`는 `SUPERSEDED`. 콘솔 **정책** 메뉴에서 조작 가능. (B1-2 평가 엔진 연동은 별도)
...
| **미구현·스텁** | Admin **역할별** RBAC·2인 승인, 정책 **평가 엔진**(Spend/issue 연동), 예외 큐, Conversion Router 실연동(DEX/CEX), `paypoint-worker`, Mobile 셸 |
