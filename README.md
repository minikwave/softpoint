...
- **랜딩** (`/`): 마케팅/진입. 공통 헤더로 **제품** (`/product`)·**개발자** (`/developers`)·**디앱**. MVP 현황 pill·쌓음/PayPoint 구분 문구. **디앱 들어가기** → `/app`, **가게 관리** → `/app/store`
...
- [**배포 (Supabase · Railway · Vercel)**](./docs/DEPLOY.md) — DB 링크, API·웹 프로덕션 URL, `prisma db push` 안내
- [**MVP 잔여·대사/모니터링 현황**](./docs/MVP_REMAINING_AND_RECONCILIATION.md) — §7 미완 항목, 은행·온체인·PCI 연동 대사 인프라 **미구현** 정리
